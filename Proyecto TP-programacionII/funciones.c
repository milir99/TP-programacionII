...
//Mostrar lista de practicas antes de ingresos o cambios
/*solicita al usuario una decisión ('s' o 'n'). Si la entrada es incorrecta, muestra un mensaje de error y vuelve a solicitar la elección.
 Si la elección es 'n', muestra la lista de prácticas ordenada por nombre.*/
int mostrarListaDePracticasSiHayError(nodoPracticasLaboratorio* listaPracticas, char nombrePractica[])
{
    ...
    if(existe==NULL)
    {
        ...
        printf("Si desea ver la lista de practicas ingrese 's' de lo contrario ingrese 'n'.\n");
        fflush(stdin);
        scanf("%c",&eleccion);
...
/*Recibe una lista de prácticas de laboratorio y un perfil, crea una nueva lista ordenada por nombre y muestra la información según el perfil,
...
void mostrarListaPorNombre(nodoPracticasLaboratorio *listaPracticas, int perfil)
{
    ...
    nodoPracticasLaboratorio *listaOrdenada = NULL;

    nodoPracticasLaboratorio *aux = listaPracticas;
    while (aux != NULL)
    {
        listaOrdenada = insertarNodo(listaOrdenada, aux->datos);
        aux = aux->siguiente;
    }

    mostrarListaPracticas(listaOrdenada, perfil);

    liberarLista(listaOrdenada);
}
//FUNCION INSERTAR Nodo
/*Inserta el nodo  por nombre de practica en orden alfabetico*/
nodoPracticasLaboratorio *insertarNodo(nodoPracticasLaboratorio *listaOrdenada, practicasLaboratorio datos)
{
    nodoPracticasLaboratorio *nuevoNodo = CrearNodoPracticaLaboratorio(datos);

    if (listaOrdenada == NULL || strcmp(datos.nombreDePractica, listaOrdenada->datos.nombreDePractica) < 0)
    {
    ...
    while (actual->siguiente != NULL &&
            strcmp(datos.nombreDePractica, actual->siguiente->datos.nombreDePractica) > 0)
    {
...
//FUNCION LIBERAR LA LISTA
/*Libera la lista que fue creada como auxiliar*/
void liberarLista(nodoPracticasLaboratorio *lista)
{
    while (lista != NULL)
    {
        nodoPracticasLaboratorio *temp = lista;
...
nodoPracticasLaboratorio* baja_de_practicasLaboratorio(nodoPracticasLaboratorio*listaPracticas,nodoArbolPacientes* arbolPacientes)
{
    ...
    do
    {
        ...
        printf("\nIngrese nombre de la practica a dar de baja: ");
        fflush(stdin);
        ...
        else
        {
            ...
            clearScreen();
            correcto=mostrarListaDePracticasSiHayError(listaPracticas,practicaACambiar);
            system("pause");
        }
...
//Pasar datos del archivo a las practicas
/*lee un archivo binario de prácticas de laboratorio. Si existe, carga los datos en una lista enlazada.
 Si no existe, crea el archivo y retorna la lista. No modifica la lista original.*/
...
nodoPracticasLaboratorio* modificacion_de_practica(nodoPracticasLaboratorio* lista)
{
    ...
    do
    {
        ...
        {
            printf("Entrada no valida. Por favor, reintentelo.\n");
            //mostrarListaDePracticasSiHayEIngrese nombre de la pr├íctica que desea modificarrror(lista);
            correcto = 1;
        }
        else
        {
            ...
            clearScreen();
            correcto=mostrarListaDePracticasSiHayError(lista,nombrePractica);
            system("pause");
        }
...
nodoPracticasXIngreso*BuscarPXIparaModificar( nodoArbolPacientes*arbol,nodoPracticasLaboratorio*listaPracticas)
{
    ...
    char nombrePractica[30];
    int  correcto ;
    nodoPracticasXIngreso *seg=NULL;
    ...
    {
        printf("No se encontro un ingreso con ese numero.\n");
    }
    else
    {
        clearScreen();
        mostrarListaPXI(existeIngreso->listaDePracticas);
        system("pause");
        ...
        do
        {
            ...
            printf("Ingrese nombre de la practica que desea modificar: ");
            fflush(stdin);
            ...
            if(correcto==0 && buscarPracticaEnIngreso(existeIngreso,existe->datos.nroPractica)==0 )
            {
        ...
        seg=existeIngreso->listaDePracticas;
        while(seg!=NULL)
        {
...
//FUNCION PARA DAR DE BAJA PXI
/*elimina una práctica de un ingreso, liberando memoria y ajustando enlaces.
 Retorna el árbol de pacientes modificado y muestra mensajes de éxito o error.*/
nodoArbolPacientes* darDeBajaPXI(nodoArbolPacientes* arbol, nodoIngresos* ingreso, nodoPracticasXIngreso** practicaXIngreso)
{
    nodoPracticasXIngreso* actual = ingreso->listaDePracticas;
    nodoPracticasXIngreso* anterior = NULL;

    // Buscar el nodo a remover en la lista
    while (actual != NULL && actual != *practicaXIngreso)
    {
        anterior = actual;
        actual = actual->siguiente;
    }
    ...
    if (actual != NULL)
    {
        ...
        {
            ingreso->listaDePracticas = actual->siguiente;
        }
        // Liberar la memoria del nodo removido
        free(actual);

        *practicaXIngreso = NULL;
        clearScreen();
        puts("\n<<>><<>><<>><<>><<>><<>><<>><<>><<>>");
        printf("La practica fue dada de baja exitosamente.\n");
        puts("\n<<>><<>><<>><<>><<>><<>><<>><<>><<>>");
        system("pause");
    }
    else
    {
        clearScreen();
        puts("\n<<>><<>><<>><<>><<>><<>><<>><<>><<>><<>><<>><<>><<>>\n");
        printf("Error: La práctica no fue encontrada en la lista.\n");
        puts("\n<<>><<>><<>><<>><<>><<>><<>><<>><<>><<>><<>><<>><<>>");
        system("pause");
    }
...
// FUNCION BUSCAR PXI Y DAR DE BAJA
/*solicita el número de ingreso y el nombre de la práctica al usuario, validando las entradas. Luego, busca el ingreso y
la práctica correspondiente. Si encuentra la práctica, llama a darDeBajaPXI para eliminarla del ingreso y muestra mensajes de estado.
 Retorna el árbol de pacientes modificado.*/
nodoArbolPacientes* BuscarPXIparaDarDeBaja(nodoArbolPacientes* arbol, nodoPracticasLaboratorio* listaPracticas)
{
    ...
    do
    {
        ...
        if (scanf("%i", &nroIngreso) != 1)
        {
            printf("Entrada no válida. Por favor, reinténtelo.\n");
            correcto = 1;
        }
    }
    while (correcto == 1);
    ...
    if (existeIngreso == NULL)
    {
        printf("No se encontró un ingreso con ese número.\n");
    }
    else
    {
        clearScreen();
       mostrarListaPXIConNombre(existeIngreso->listaDePracticas,listaPracticas);
        system("pause");
        ...
        do
        {
            ...
            printf("Ingrese nombre de la practica que desea dar de baja: ");
            fflush(stdin);
            if (fgets(nombrePractica, sizeof(practicasLaboratorio), stdin) == NULL)
            {
                printf("Entrada no válida. Por favor, reinténtelo.\n");
            ...
            if (existe == NULL)
            {
                printf("La practica ingresada no existe. Por favor, inténtelo otra vez\n");
                correcto = 1;
            }
            if (correcto == 0 && buscarPracticaEnIngreso(existeIngreso, existe->datos.nroPractica) == 0)
            {
                printf("La practica no existe dentro del número de ingreso. Por favor, inténtelo otra vez\n");
                correcto = 1;
            }
        }
        while (correcto == 1);
        ...
        if (seg != NULL)
        {
            arbol = darDeBajaPXI(arbol, existeIngreso, &seg);
        }
    }
    ...
    do
    {
        do
        {
            ...
            else
            {
                ...
                clearScreen();
                correcto = mostrarListaDePracticasSiHayError(listaPracticas,nombrePractica);
                system("pause");
            }
        }
        while(correcto==1);
